...
Phase 5: adds Skill injection into system prompt.
"""
...
# ---------------------------------------------------------------------------
# Default system prompt (base — skills are appended)
# ---------------------------------------------------------------------------

_BASE_CODE_AGENT_PROMPT = """你是一个专业的 Python 编程助手。
...
重要原则：
- 先了解项目结构和现有代码再动手
- patch 工具要求 old_string 和 new_string 同时提供，且 old_string 必须精确匹配
- 写完代码后运行测试验证正确性
- 保持代码简洁、可读、符合 PEP 8
- 如果任务不明确，先描述你的理解再开始
...
class CodeAgent(SpecialistAgent):
    ...
    Uses the ReAct (Reason + Act + Observe) pattern.
    Skills are injected into the system prompt for domain-specific expertise.
    """
    ...
    def __init__(
        ...
        *,
        ...
        skills: list["SkillInstance"] | None = None,
    ) -> None:
        ...
        self._skills: list["SkillInstance"] = skills or []

        # System prompt = base + skill prompts
        self._system_prompt = system_prompt or _BASE_CODE_AGENT_PROMPT
    ...
    def set_skills(self, skills: list["SkillInstance"]) -> None:
        """Update the active skills and rebuild the system prompt."""
        self._skills = skills
        # Rebuild executor with new prompt
        self._executor = ReActExecutor(
            client=self._client,
            system_prompt=self._build_system_prompt(),
    ...
    def _build_system_prompt(self) -> str:
        """Assemble the full system prompt: base + all skill prompts."""
        parts = [self._system_prompt]
        for si in self._skills:
            if si.active:
                parts.append(f"\n\n{'='*40}\n")
                parts.append(f"# Skill: {si.name}\n")
                parts.append(si.prompt)
        return "\n".join(parts)
